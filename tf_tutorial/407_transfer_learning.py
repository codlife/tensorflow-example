...
def load_img(path):
    ...
    resized_img = skimage.transform.resize(crop_img, (1,224, 224,3))  # shape [1, 224, 224, 3]
...
class Vgg16:
    ...
    def predict(self, paths):
        ...
        for i, path in enumerate(paths):
            print(path)
            x = load_img(path)
            # temp = self.sess.run(self.bgr,{self.tfx: x})
            # print(temp)
            length = self.sess.run(self.out, {self.tfx: x})
            img = skimage.io.imread(path)
            axs[i//3][i%3].imshow(img)
            axs[i//3][i%3].set_title('Len: %.1f cm' % length)
...
def eval():
    ...
    vgg.predict(
        ['./for_transfer_learning/data/kittycat/000129037.jpg',
         './for_transfer_learning/data/kittycat/244730.jpg',
         './for_transfer_learning/data/kittycat/252722.jpg',
         './for_transfer_learning/data/tiger/391412.jpg',
         './for_transfer_learning/data/tiger/busterL2108_468x655.jpg',
         './for_transfer_learning/data/tiger/ExoticAnimals3.jpg'
         ])
