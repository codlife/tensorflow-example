...
#     l_eval = tf.layers.dense(tf_s, 10, tf.nn.relu, kernel_initializer=
#                              tf.random_normal_initializer(0,0.1))
#     q = tf.layers.dense(l_eval,N_ACTIONS, kernel_initializer=
#                         tf.random_normal_initializer(0,0.1))
...
# a_indices = tf.stack([tf.range(tf.shape(tf_a)[0],dtype = tf.int32,
#                                ),tf_a], axis = 1)
...
#                                      scope = 'q')
#         sess.run([tf.assign(t,e) for t, e in zip(t_params,
#                                                  e_params)])
